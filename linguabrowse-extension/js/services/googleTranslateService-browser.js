/**
 * Google Translate Service for LinguaBrowse (Browser-compatible version)
 * For use in standalone testing environments outside the extension
 */
...
class GoogleTranslateService {
  constructor() {
    ...
    console.log("[Google Translate Service] Initialized (browser version)");
  }
  ...
  async translateText(text, targetLang, sourceLang = 'english') {
    console.log(`[Google Translate] Translating "${text}" from ${sourceLang} to ${targetLang}`);
    ...
    try {
      // In standalone testing, we only use the simulated translations
      ...
      console.log(`[Google Translate] Translated "${text}" to "${translation}"`);
      return translation;
    } catch (error) {
  ...
  /**
   * Simulate translation for testing (without API calls)
...
// Create singleton instance and expose to window for testing
